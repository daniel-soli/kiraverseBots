...
namespace KiraVerse.Bots.Helpers;
...
public static class TokenAmountConverter
{
    ...
    public static string ToString(decimal? balance, int? decimals, decimal conversionRate = 1)
    {
        if (balance == null) return "";

        //decimal floatBalance = ToDecimal(balance, decimals, conversionRate);

        decimal floatBalance = (decimal)((double)balance / Math.Pow(10, (double)decimals!));
